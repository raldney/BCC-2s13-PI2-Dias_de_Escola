void carregarMenu()
{
menuDisplay = al_create_display(LARGURA_TELA, ALTURA_TELA);
    if (!menuDisplay)
    {
   ...
   /* inicialSound = al_create_sample_instance(al_load_sample("Musicas/musicaMenu.ogg"));
    ...
    //Criando o Fundo
    fundoMenu = al_load_bitmap("Imagens/Menu/sala.bmp");
    if(!fundoMenu)
    {
    ...
    al_set_window_title(menuDisplay,"Scholl Days");
    ...
    al_register_event_source(eventosQueue, al_get_display_event_source(menuDisplay));
